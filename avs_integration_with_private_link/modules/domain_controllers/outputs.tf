...
output "domain_distinguished_name" {
  ...
  value = local.domain_distinguished_name
}
...
output "ldap_user" {
  ...
  value       = "azureuser@${local.domain_fqdn}"
}
...
output "dns_server_ips" {
  ...
  value       = [module.testvm.virtual_machine_azurerm.private_ip_address, module.testvm_secondary.virtual_machine_azurerm.private_ip_address]
}
